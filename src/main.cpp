...
bool validateSettings(const BongoCatSettings *s)
{
    ...
    if (s->sleep_timeout_minutes < 1 || s->sleep_timeout_minutes > 60)
        return false;
    if (s->animation_sensitivity < 0.1 || s->animation_sensitivity > 5.0)
        return false;
...
void handleSerialCommands()
{
    if (Serial.available())
    {
        ...
        if (command.startsWith("SPEED:"))
        {
            ...
            { // Slow threshold: <20 WPM -> speed 80+
                new_state = ANIM_STATE_TYPING_SLOW;
            }
            else if (speed < 150)
            { // Normal threshold: 20-40 WPM -> speed 80-150
                new_state = ANIM_STATE_TYPING_NORMAL;
            }
            else
            { // Fast threshold: 40+ WPM -> speed 150+
        ...
        else if (command.startsWith("STATS:"))
        {
            ...
            int cpuTemp = 0;
            if (cpuTempStart > 7 && cpuTempEnd > cpuTempStart)
            { // Check if CPUTemp: was found
            ...
            int gpuTemp = 0;
            if (gpuTempStart > 7 && gpuTempEnd > gpuTempStart)
            { // Check if GPUTemp: was found
    ...
    if (manager->current_state == ANIM_STATE_IDLE_STAGE4)
    {
        ...
        { // Change effect every second
            ...
            case 0:
                ...
                break;
            case 1:
                manager->current_sprites[LAYER_EFFECTS] = &sleepy2;
                break;
            case 2:
                manager->current_sprites[LAYER_EFFECTS] = &sleepy3;
                break;
            }
...
const char *get_state_name(animation_state_t state)
{
    ...
    case ANIM_STATE_TYPING_FAST:
        return "TYPING_FAST";
    case ANIM_STATE_TYPING_STREAK:
        return "TYPING_STREAK"; // Keep for compatibility but unused
    default:
        return "UNKNOWN";
    }
}
...
void loop()
{
    ...
    { // 50 FPS max for LVGL (was every 5ms)
        lv_timer_handler();
